...
% ===============================================================
\section{Trabajo Futuro y Objetivos Experimentales}
% ===============================================================

Aunque el alcance de este documento es la revisión del estado del arte, conviene explicitar los objetivos experimentales que guiarán las entregas posteriores del reto ---en particular el documento Fase~2A--- para asegurar la trazabilidad entre la evidencia revisada y las decisiones de diseño.

\textbf{Objetivos de la Fase~2 (entrega 2A).} (i) Construir la pipeline reproducible de preprocesamiento, particionado y evaluación sobre eRisk~2018 Tarea~2 anorexia, fijando las particiones canónicas (152 train / 320 test) y las métricas comunes a todos los experimentos (\textbf{AUC, F1, precisión, recall, ERDE-5 y ERDE-50}). (ii) Implementar como referencias clásicas: TF-IDF+LR, Word2Vec promediado por usuario, LIWC+LR, BoSE+LR (réplica de Aragón et al.~\cite{aragon2023bose}) y un OCC-kSS ablativo siguiendo a Aguilera et al.~\cite{aguilera2021oneclass}. (iii) Reportar el primer punto de la curva clásico--transformer mediante el AUC de cada referencia.

\textbf{Objetivos de la Fase~3.} (i) Ajustar BERT-base con la receta UNSL\#1~\cite{thompson2024timeaware} (50 tokens SS3 anexados al vocabulario, AdamW, LR 3e-5, batch~8, 5 epochs) y MentalBERT como variante específica de dominio. (ii) Implementar el comparador LLM zero-shot resume-y-prompt de Saini y Sen~\cite{saini2026patchtst} sobre LLaMA-3 8B o Mistral-7B. (iii) Producir las curvas ROC cabeza a cabeza y un análisis de error cualitativo sobre los falsos negativos, dado que constituyen el modo de fallo clínicamente más costoso.
...
Más allá de los números, los hallazgos revisados subrayan que el diseño responsable de sistemas de detección de desórdenes alimenticios debe priorizar la \emph{interpretabilidad}, la \emph{calibración a costos clínicos asimétricos} ---donde un falso negativo puede ser irreversible--- y la \emph{permanencia del juicio humano} (clínico, moderador o paciente) en el bucle de decisión. La detección automática no debe entenderse como sustituto del diagnóstico clínico, sino como una herramienta de tamizaje y alerta temprana cuyo valor real depende de su integración ética con los flujos de atención y de la protección de la privacidad de poblaciones vulnerables.
