...
class TestEvaluator:
    ...
    def test_export_clinical_errors_content(self, mock_results, tmp_path):
        """Verifica que el Excel de errores contenga los datos correctos."""
        y_true, y_pred, y_probs, texts, _ = mock_results
        filename = tmp_path / "falsos_negativos.xlsx"
        
        export_clinical_errors(texts, y_true, y_pred, y_probs, filename=str(filename))
        
        assert os.path.exists(filename)
        df_error = pd.read_excel(filename)
    ...
    def test_metrics_calculation_consistency(self, mock_results, monkeypatch):
        """Smoke test para asegurar que el evaluador no falle con modelos lineales."""
        y_true, y_pred, y_probs, texts, features = mock_results
        
        class MockLinearModel:
            coef_ = [np.random.rand(10)]
        
        # Evitar que se abran ventanas de GUI durante los tests automatizados
        monkeypatch.setattr(plt, "show", lambda: None)
        monkeypatch.setattr(plt, "savefig", lambda x: None)
        
        # Si la función termina, las métricas internas de sklearn funcionaron
        evaluate_model(y_true, y_pred, y_probs, MockLinearModel(), features, texts)